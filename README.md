...
执行真实 SQL Server 多实例对账验收（会创建并自动销毁临时容器、数据库和五个 API 进程）：
...
脚本要求 Docker Desktop 已运行，且指定的 SQL 与五个连续 API 端口均可绑定。它真实执行 001–005 迁移，使用不同身份完成申请、独立业务审批、`OutcomeUnknown` 冻结、第一人复核，随后强制终止首次调用实例和第一复核实例；两个剩余 API 实例并发争抢第二人裁决，必须恰好一个成功。最后启动同身份替代实例验证滚动部署后返回 `Reconciled`，并确认工具没有再次执行。脚本不输出数据库密码，失败时保留诊断日志路径，成功后自动删除临时资源。
...
1. 增加“工具已经进入 `Executing` 后立即强杀进程”的可控故障注入验收；多实例并发裁决、已冻结状态后的进程强杀和滚动回放已通过真实容器验证。
...
- 2026-07-14 本地自动化测试 80/80 通过；真实 SQL Server LocalDB 测试执行 `001` 至 `005`，验证执行结果加密回放、`OutcomeUnknown` 冻结、租户隔离、目标探测、两名不同人员的原子裁决和已生效结案。Docker 临时 SQL Server 与五个真实 API 进程进一步验证：不同身份跨实例审批、调用实例强杀、第一复核实例强杀、两个第二复核实例并发时恰好一个胜出，以及滚动替代实例返回 `Reconciled`。尚未完成的是在工具处于 `Executing` 的精确窗口内强杀。150 条离线评测决策匹配率 90%、引用召回率 75%，质量门禁通过；NuGet 直接与传递依赖未发现已知漏洞。
