...
这是 AI Agent 系统的第一条可运行纵切：身份与用户组进入系统后，依次经过输入安全审核、ACL 前置过滤检索、确定性证据重排、独立证据充分性门禁、Microsoft Agent Framework 回答编排、结构化引用、运行轨迹和离线测评。
...
- `src\AiMentor.Infrastructure`：Markdown/OpenSearch RAG、证据重排与充分性判定、安全规则、轨迹、Agent Framework 和确定性模型适配器。
...
2026-07-13 的当前可复现基线：150 条全部执行，决策匹配率 89.33%，复合来源按“全部命中”计算的用例级引用召回率 71.25%；无答案、记忆、安全、ACL 四类决策匹配率均为 100%。与加入独立充分性门禁前相比，决策匹配率由 87.33% 提升到 89.33%，无答案类由 93.33% 提升到 100%，记忆类由 60% 提升到 100%；引用召回率由 72.50% 降至 71.25%，原因是更严格的拒答不再携带引用，而不是错误引用增加。架构类仍仅为 26.67%，主要原因是这些题引用总体设计文档章节，而当前 RAG 范围刻意只加载 23 份知识包文档；事故复合引用召回率仍为 33.33%。这些指标是阶段性回归基线，不是生产上线验收结论。
...
1. 将规则审核扩展为输入、检索内容、工具参数、模型输出四道审核，并接入策略版本管理；优先补齐模型输出引用一致性和工具参数越权检测。
2. 建立显式记忆授权工作流，区分会话记忆、用户偏好和长期事实，覆盖写入确认、查看、更正、删除、过期和租户隔离。
