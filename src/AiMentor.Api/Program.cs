...
// V0 兼容入口不再出现在 OpenAPI，新调用方统一使用 /api/v1。
app.MapGet("/api/knowledge/stats", () => Results.Ok(repository.Statistics)).ExcludeFromDescription();
app.MapPost("/api/questions", AskAsync).RequireRateLimiting("questions").ExcludeFromDescription();
app.MapPost("/api/questions/stream", StreamAsync).RequireRateLimiting("questions").ExcludeFromDescription();

await app.RunAsync();

static async Task<IResult> AskAsync(AskRequest request, ITrustedQuestionService service, HttpContext context,
    CancellationToken cancellationToken)
{
    var correlationId = GetCorrelationId(context);
    var answer = await service.AskAsync(new TrustedQuestion(request.Question,
        AccessContext.Create(request.TenantId, request.SubjectId, request.Groups), correlationId), cancellationToken);
...
static async Task StreamAsync(AskRequest request, ITrustedQuestionService service, HttpContext context,
